...
class _NotePageState extends State<NotePage> {
            ...
            Tooltip(
              ...
              child: IconButton(
                icon: Icon(
                  PhosphorIcons.pen(),
                  color:
                      currentMode == DrawMode.pen ? Colors.blue : Colors.black,
                ),
                onPressed: () {
                  setState(() {
                        ...
                        child: ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            return GestureDetector(
                              onTap: () async {
                                final oldPageId =
                                    pages[currentPageIndex]['page_id'];
                              ...
                              child: Container(
                                ...
                                child:
                                    previewUrl != null
                                        ? CachedNetworkImage(
                                          imageUrl:
                                              '$baseUrl$previewUrl?timestamp=${DateTime.now().millisecondsSinceEpoch}',
                    ...
                    Expanded(
                      child: PageView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          final pdfImageUrl =
                              '$baseUrl/pdf/page-image/${widget.pdfId}/$pageNumber';
                          final rawAspectRatio =
                              pages[index]['aspect_ratio'] ?? 0.75;
                          final aspectRatio =
                              rawAspectRatio > 0 ? rawAspectRatio : 0.75;
                                ...
                                child: Center(
                                  child: Listener(
                                    ...
                                    child: Builder(
                                      builder: (context) {
                                        debugPrint(
                                          'scale: min=${_minScales[index]}, max=4.0',
                                        );
  ...
  void _handlePanUpdate(DragUpdateDetails details, int pageId) {
    ...
    if (currentMode == DrawMode.transform && selectedIndexes.isNotEmpty) {
      if (_isScaling && _scaleDragStart != null) {
        final scenePrev = _controllers[currentPageIndex].toScene(
          details.localPosition,
        );
        final sceneCurr = _controllers[currentPageIndex].toScene(
  ...
  void _scaleSelectionGesture(double factor) {
    ...
    switch (_activeHandleIndex) {
      ...
      case 3:
        ...
        break;
      default:
        anchor = _computeCenter(selectedIndexes, strokes);
    }
  ...
  void _clearSelectionAtTapPosition(Offset tapPosition) {
    ...
    final selectionRect = Rect.fromPoints(
      selectedPoints.reduce(
        (a, b) => Offset(a.dx < b.dx ? a.dx : b.dx, a.dy < b.dy ? a.dy : b.dy),
      ),
      selectedPoints.reduce(
        (a, b) => Offset(a.dx > b.dx ? a.dx : b.dx, a.dy > b.dy ? a.dy : b.dy),
      ),
    ...
    final tappedOnHandle = handles.any(
      (handle) => Rect.fromCenter(
        center: handle,
        width: handleHitBoxSize,
        height: handleHitBoxSize,
  ...
  Offset _computeCenter(Set<int> indexes, List<Stroke> strokes) {
    ...
    final dx =
        allPoints.map((p) => p.dx).reduce((a, b) => a + b) / allPoints.length;
    final dy =
        allPoints.map((p) => p.dy).reduce((a, b) => a + b) / allPoints.length;
    return Offset(dx, dy);
  }
  ...
  void _selectByLasso() {
    ...
    debugPrint(
      "라쏘 선택 시작 - 라쏘 포인트 개수: ${lassoPoints.length}, 스토로크 수: ${strokes.length}",
    );

    // 라쏘 좌표를 scene 기준으로 변환
    final transformedLasso =
        lassoPoints
            .map((p) => _controllers[currentPageIndex].toScene(p))
            .toList();
    ...
    for (int i = 0; i < strokes.length; i++) {
      ...
      bool pointInside = stroke.points.any(
        (point) => _pointInPolygon(point, transformedLasso),
      );

      // 2. 중심점이 라쏘 경계 근처에만 있어도 선택되도록 보완
      final center = _computeCenter({i}, strokes);
      final nearLasso = transformedLasso.any(
        (p) => (p - center).distance < 15,
      ); // ← 거리 임계값 (조절 가능)
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (contextBox != null) {
        ...
        if (box != null && box.hasSize) {
          ...
          debugPrint(
            'viewerSize: \${viewerSize.width} x \${viewerSize.height}',
          );
          debugPrint(
            'pageSize: \$pageWidth x \$pageHeight (aspectRatio: \$aspectRatio)',
          );
  ...
  Future<void> _fetchPages() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final newViewerKeys = List.generate(
          newPages.length,
          (_) => GlobalKey(),
        );
        final newRepaintKeys = List.generate(
          newPages.length,
          (_) => GlobalKey(),
        );
  ...
  Future<void> _loadAnnotations(int pageId) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final aspectRatio =
            pages.firstWhere((p) => p['page_id'] == pageId)['aspect_ratio'] ??
            0.75;
        ...
        for (var annotation in data) {
          if (annotation['data']?['lines'] != null) {
            for (var line in annotation['data']['lines']) {
              ...
              pageStrokes[pageId]!.add(
                Stroke(points: strokePoints, paint: paint, penType: penType),
              );
            }
          }
        }
      ...
      child: SizedBox(
        ...
        child: GestureDetector(
          ...
          onTapDown: (details) {
            ...
            final tappedHandle = handles.any(
              (handle) => Rect.fromCenter(
                center: handle,
                width: handleHitBoxSize,
                height: handleHitBoxSize,
  ...
  void _pasteSelection() {
    ...
    selectedIndexes =
        List.generate(copiedStrokes.length, (i) => startIndex + i).toSet();
  ...
  void _showPasteMenu(Offset globalPosition) {
    showMenu(
      ...
      items: [PopupMenuItem(child: const Text('붙여넣기'), onTap: _pasteSelection)],
    );
  }
}
...
class _LassoNotePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (lassoPoints.isNotEmpty) {
      ...
      final sceneLassoPoints =
          lassoPoints.map((p) => controller.toScene(p)).toList();
