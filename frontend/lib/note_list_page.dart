...
const String baseUrl = 'http://3.107.195.136:8004'; // 또는 실제 서버 주소
...
class _NoteListPageState extends State<NoteListPage> {
  ...
  Future<void> fetchNotes() async {
    ...
    try {
      final response = await http.get(
        Uri.parse('http://3.107.195.136:8004/pdf/notes/${widget.folderId}'),
        headers: {
  ...
  Future<void> createNote() async {
    ...
    try {
      final response = await http.post(
        Uri.parse('http://3.107.195.136:8004/pdf/notes'),
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Bearer $accessToken',
        },
  ...
  Future<void> uploadPdfFile() async {
    ...
    if (pickedFile != null && pickedFile.path.isNotEmpty) {
      ...
      final accessToken = await getAccessToken();
      final uri = Uri.parse('http://3.107.195.136:8004/pdf/upload');
...
class _NoteItemCardState extends State<NoteItemCard> {
  ...
  Future<void> loadThumbnailOrPageId() async {
    ...
    try {
      // 썸네일 먼저 시도
      final response = await http.get(
        Uri.parse('http://3.107.195.136:8004/pdf/pages/${widget.note.id}'),
        headers: {
          'Authorization': 'Bearer $accessToken',
        },
      );
      ...
      if (response.statusCode == 200) {
        ...
        if (pages.isNotEmpty) {
          ...
          setState(() {
            if (imageUrl != null) {
              thumbnailUrl = 'http://3.107.195.136:8004$imageUrl';
            } else {
  ...
  Future<void> _showMoveDialog(int noteId) async {
    ...
    final response = await http.get(
      Uri.parse('http://3.107.195.136:8004/pdf/folders'),
      headers: {'Authorization': 'Bearer $token'},
    );
    ...
    final response = await http.patch(
      Uri.parse('http://3.107.195.136:8004/pdf/notes/$noteId'),
      headers: {
