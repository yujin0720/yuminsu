...
class _StudyPreferencePageState extends State<StudyPreferencePage> {
  ...
  final Map<int, String> timeOptions = {
    ...
    for (int i = 60; i <= 600; i += 30)
      i: i % 60 == 0 ? "${i ~/ 60}시간" : "${i ~/ 60}시간 ${i % 60}분",
  };
  ...
  Future<void> _submitPreferences() async {
    ...
    if (response.statusCode == 200) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text("공부 목표 시간이 저장되었습니다.")));
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text("공부 목표 시간 입력"), centerTitle: true),
      body: Padding(
        ...
        child: Column(
          children: [
            ...
            Expanded(child: SingleChildScrollView(child: _buildDropdownList())),
