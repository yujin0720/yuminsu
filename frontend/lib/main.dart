...
class HomePageState extends State<HomePage> {
  ...
  Future<void> _submitPersonalSchedule(
    ...
    final res = await http.post(
      ...
      body: json.encode({
        ...
        'color':
            '#${color.value.toRadixString(16).padLeft(8, '0').substring(2)}', // ARGB → RGB hex
      }),
    );
  ...
  Future<void> fetchTodayTodos() async {
    ...
    final planRes = await http.get(
      Uri.parse('$baseUrl/plan/today?date=$today'),
      headers: headers,
    );
    ...
    final personalRes = await http.get(
      Uri.parse('$baseUrl/personal-schedule/today'),
      headers: headers,
    );
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          if (title.isNotEmpty) const SizedBox(height: 20), //12
          child,
        ],
      ),
    );
        ...
        Expanded(
          ...
          child: Consumer<TimerProvider>(
            builder: (context, timer, _) {
              final today =
                  ['월', '화', '수', '목', '금', '토', '일'][DateTime.now().weekday -
                      1];
              final todayStudyMin = timer.weeklyStudy[today]?.inMinutes ?? 0;
              final weeklyStudyMin = timer.weeklyStudy.values.fold<int>(
                0,
      ...
      child: TableCalendar(
        ...
        onDaySelected: (selectedDay, focusedDay) {
          ...
          _showFullTodoPopup(
            context,
            selectedDay,
            _eventDataMap[dateKey] ?? [],
          );
        },
        eventLoader:
            (day) => _events[DateTime.utc(day.year, day.month, day.day)] ?? [],
        ...
        calendarBuilders: CalendarBuilders(
          ...
          todayBuilder:
              (context, day, _) => _buildCalendarDay(day, isToday: true),
        ),
      ),
    );
  }
