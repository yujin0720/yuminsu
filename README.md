...
- `edit_profile_page.dart`  
  사용자 이름, 이메일, 연락처 수정 및 비밀번호 변경 화면

- `folder_home_page.dart`  
  필기 노트 목록을 폴더 형태로 관리하는 홈 화면

- `login_page.dart`  
  로그인 화면 (ID, 비밀번호 입력 및 토큰 저장 처리 포함)

- `main.dart`  
  앱 실행 진입점 및 라우팅 설정

- `mypage.dart`  
  마이페이지 - 사용자 프로필, 주간 공부 시간, 실제 공부 시간 확인

- `note_list_page.dart`  
  특정 폴더 내의 노트 리스트를 보여주는 화면

- `note_page.dart`  
  PDF 필기 기능 제공 - 하이라이트, 펜, 지우개, 썸네일 생성 등 포함

- `password_check_page.dart`  
  민감 정보 접근 전 비밀번호 재확인 페이지

- `signup_page.dart`  
  회원가입 화면 (생년월일, 전화번호, 비밀번호 등 입력)

- `splash_page.dart`  
  앱 실행 시 첫 화면, 토큰 유무에 따라 자동 로그인 처리

- `study_result.dart`  
  학습 완료 체크 화면 - 주차별 항목 체크 UI 제공

- `studyplan.dart`  
  시험 정보와 공부 자료 입력 → AI가 계획 자동 생성

- `submain.dart`  
  주간 학습 계획 화면 (요일별 체크박스 및 계획 수정 UI)

- `timer.dart`  
  공부 시간 측정용 타이머 UI 화면

- `timer_provider.dart`  
  타이머 상태 관리, 누적 시간 계산, 서버 저장 및 불러오기 기능

- `todo_provider.dart`  
  오늘/주간 할 일 상태 관리 및 서버 동기화 처리
