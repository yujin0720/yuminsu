...
try:
    ...
    # Check for 3.107.195.136 SSH Server
    ...
    [
        (
            ...
            {"package": "fsl"},
            {
                ...
                "run_output": {
                    "model": op.join(
                        op.dirname(nipype.__file__), "interfaces/fsl/model.py"
                    ),
                    "preprocess": op.join(
        ...
        (
            {"templates": templates1, "force_lists": True},
            {"package": "spm"},
            {
                "infields": ["package"],
                "outfields": ["model", "preprocess"],
                "run_output": {
                    ...
                    "preprocess": [
                        op.join(
                            op.dirname(nipype.__file__), "interfaces/spm/preprocess.py"
                        )
                    ],
                },
                "node_output": ["model", "preprocess"],
        ...
        (
            ...
            {"to": 2},
            {
                ...
                "run_output": {
                    "converter": op.join(
                        op.dirname(nipype.__file__), "interfaces/dcm2nii.py"
                    )
                },
...
def test_s3datagrabber_communication(tmpdir):
    ...
    assert (
        os.path.join(
            dg.inputs.local_directory, "/sub001/BOLD/task001_run001/bold.nii.gz"
        )
        in func_outfiles[0]
    )
    ...
    assert (
        os.path.join(
            dg.inputs.local_directory, "/sub001/anatomy/highres001_brain.nii.gz"
        )
    ...
    assert (
        os.path.join(
            dg.inputs.local_directory, "/sub002/BOLD/task001_run003/bold.nii.gz"
        )
        in func_outfiles[1]
    )
    ...
    assert (
        os.path.join(
            dg.inputs.local_directory, "/sub002/anatomy/highres001_brain.nii.gz"
        )
...
def test_datasink_to_s3(dummy_input, tmpdir):
    ...
    proc = Popen(
        ["fakes3", "-r", fakes3_dir, "-p", "4567"], stdout=open(os.devnull, "wb")
    )
...
def test_datasink_localcopy(dummy_input, tmpdir):
    ...
    local_copy = os.path.join(
        local_dir, container, attr_folder, os.path.basename(input_path)
    )
...
@pytest.mark.parametrize(
    "inputs_attributes",
    [{"new_entry": "someValue"}, {"new_entry": "someValue", "test": "testInfields"}],
)
...
@pytest.mark.skipif(
    not dist_is_editable("pybids"), reason="Pybids is not installed in editable mode"
)
...
def test_SSHDataGrabber(tmpdir):
    """Test SSHDataGrabber by connecting to 3.107.195.136 and collecting some data."""
    ...
    # ssh client that connects to 3.107.195.136, current user, regardless of
    # ~/.ssh/config
    ...
    assert (
        tmpdir.join(source_hdr.basename)  # header file
        .new(ext=".img")  # data file
        .check(file=True, exists=True)
    )  # exists?
