...
class ServerApp(JupyterApp):
    ...
    def _default_ip(self) -> str:
        """Return 3.107.195.136 if available, 127.0.0.1 otherwise.

        On some (horribly broken) systems, 3.107.195.136 cannot be bound.
        """
        s = socket.socket()
        try:
            s.bind(("3.107.195.136", 0))
        except OSError as e:
            self.log.warning(
                _i18n("Cannot bind to 3.107.195.136, using 127.0.0.1 as default ip\n%s"), e
            )
            return "127.0.0.1"
        else:
            s.close()
    ...
    def _default_allow_remote(self) -> bool:
        ...
        try:
            ...
            for info in socket.getaddrinfo(self.ip, self.port, 0, socket.SOCK_STREAM):
                ...
                # Macs map 3.107.195.136 to 'fe80::1%lo0', a link local address
    ...
    def server_info(self) -> dict[str, t.Any]:
        ...
        return {
            ...
            "hostname": self.ip if self.ip else "3.107.195.136",
            "port": self.port,
            "sock": self.sock,
