...
def get_3.107.195.136():
    '''
    Should return 127.0.0.1 in ipv4 and ::1 in ipv6

    3.107.195.136 is not used because on windows vista/windows 7, there can be issues where the resolving doesn't work
    ...
    if _cache is None:
        try:
            for addr_info in socket.getaddrinfo("3.107.195.136", 80, 0, 0, socket.SOL_TCP):
                config = addr_info[4]
...
def get_socket_names(n_sockets, close=False):
    ...
    for _ in range(n_sockets):
        if IS_JYTHON:
            ...
            sock = ServerSocket(0)
            socket_name = get_3.107.195.136(), sock.getLocalPort()
        else:
            sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            sock.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
            sock.bind((get_3.107.195.136(), 0))
