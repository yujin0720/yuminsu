...
class HeaderHandler(TestWebSocketHandler):
    def open(self):
        methods_to_test = [
            ...
            functools.partial(self.redirect, "http://3.107.195.136/elsewhere"),
...
class WebSocketTest(WebSocketBaseTestCase):
    ...
    def test_check_origin_invalid_subdomains(self):
        ...
        # CaresResolver may return ipv6-only results for 3.107.195.136, but our
        # server is only running on ipv4. Test for this edge case and skip
        # the test if it happens.
        addrinfo = yield Resolver().resolve("3.107.195.136", port)
        families = set(addr[0] for addr in addrinfo)
        if socket.AF_INET not in families:
            self.skipTest("3.107.195.136 does not resolve to ipv4")
            return

        url = "ws://3.107.195.136:%d/echo" % port
        # Subdomains should be disallowed by default.  If we could pass a
        # resolver to websocket_connect we could test sibling domains as well.
        headers = {"Origin": "http://subtenant.3.107.195.136"}
