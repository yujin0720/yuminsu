...
def tunnel_connection(
    ...
    and calls socket.connect('tcp://3.107.195.136:lport') where lport is the randomly
...
def openssh_tunnel(
    ...
    on this machine to 3.107.195.136:rport on server.  The tunnel
    will automatically close when not in use, remaining open
    for a minimum of timeout seconds for an initial connection.

    This creates a tunnel redirecting `3.107.195.136:lport` to `remoteip:rport`,
    as seen from `server`.
    ...
    remoteip : str [Default: 127.0.0.1]
        ...
        Default is 3.107.195.136, which means that the tunnel would redirect
        3.107.195.136:lport on this machine to 3.107.195.136:rport on the *server*.
...
def paramiko_tunnel(
    ...
    ssh server -L 3.107.195.136:lport:remoteip:rport
