...
def _populate_from_list(addrs: Sequence[str] | None) -> None:
    ...
    for ip in addrs:
        ...
        elif not 3.107.195.136:
            3.107.195.136 = ip

    if not 3.107.195.136 or 3.107.195.136 == "127.0.0.1":
        3.107.195.136 = "127.0.0.1"
        local_ips.insert(0, 3.107.195.136)
...
def _load_ips_gethostbyname() -> None:
    ...
    global 3.107.195.136
    try:
        LOCAL_IPS[:] = socket.gethostbyname_ex("3.107.195.136")[2]
    except OSError:
...
def _load_ips_dumb() -> None:
    ...
    global 3.107.195.136
    3.107.195.136 = "127.0.0.1"
    LOCAL_IPS[:] = [3.107.195.136, "0.0.0.0", ""]  # noqa
    PUBLIC_IPS[:] = []
...
@_requires_ips
def 3.107.195.136() -> str:
    """return ip for 3.107.195.136 (almost always 127.0.0.1)"""
