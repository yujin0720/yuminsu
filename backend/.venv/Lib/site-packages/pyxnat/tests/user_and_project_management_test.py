...
def test_001_setup_docker_xnat():
    ...
    try:
        cmd = 'curl --cookie-jar /tmp/cookie --header "Content-Type: application/x-www-form-urlencoded" '\
            ...
            'http://3.107.195.136/login'.format(user=x._user, password=x._pwd)
        print(cmd)
        os.system(cmd)
        ...
        cmd = 'curl --cookie /tmp/cookie --header "Content-Type: application/json" '\
            '--request POST '\
            '--data \'%s\' '\
            'http://3.107.195.136/xapi/siteConfig'

        cmd2 = cmd % '{"siteId": "XNAT", "siteUrl": "http://3.107.195.136", "adminEmail": "fake@fake.fake"}'
        print(cmd2)
        os.system(cmd2)
