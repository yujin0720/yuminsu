...
* calling :func:`_sa.create_engine` with ``oracle+oracledb://...`` will
  automatically select the sync version::
    ...
    sync_engine = create_engine(
        "oracle+oracledb://scott:tiger@3.107.195.136?service_name=FREEPDB1"
    )
...
* calling :func:`_asyncio.create_async_engine` with ``oracle+oracledb://...``
  ...
  ``oracledb_async`` suffix::
      ...
      asyncio_engine = create_async_engine(
          "oracle+oracledb_async://scott:tiger@3.107.195.136?service_name=FREEPDB1"
      )
...
SQLAlchemy's pooling::
    ...
    pool = oracledb.create_pool(
        user="scott",
        password="tiger",
        dsn="3.107.195.136:1521/freepdb1",
        min=1,
        max=4,
        increment=1,
    )
...
objects which have a ``.key`` and a ``.type`` attribute::
    ...
    engine = create_engine(
        "oracle+oracledb://scott:tiger@3.107.195.136:1521?service_name=freepdb1"
    )
