...
To set isolation level using :func:`_sa.create_engine`::
    ...
    engine = create_engine(
        "mysql+mysqldb://scott:tiger@3.107.195.136/test",
...
in the URL, such as::
    ...
    e = create_engine(
        "mysql+pymysql://scott:tiger@3.107.195.136/test?charset=utf8mb4"
    )
...
string parameter ``binary_prefix=true`` to the URL to repair this warning::
    ...
    # mysqlclient
    engine = create_engine(
        "mysql+mysqldb://scott:tiger@3.107.195.136/test?charset=utf8mb4&binary_prefix=true"
    )

    # PyMySQL
    engine = create_engine(
        "mysql+pymysql://scott:tiger@3.107.195.136/test?charset=utf8mb4&binary_prefix=true"
    )
...
the ``first_connect`` and ``connect`` events::
    ...
    eng = create_engine(
        "mysql+mysqldb://scott:tiger@3.107.195.136/test", echo="debug"
    )
...
``nullable=False``.   The following example illustrates::
    ...
    e = create_engine("mysql+mysqldb://scott:tiger@3.107.195.136/test", echo=True)
    m.create_all(e)
